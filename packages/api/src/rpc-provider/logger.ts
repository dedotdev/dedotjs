...
import { formatDate, isBuffer, isFunction, isObject, isU8a, u8aToHex, u8aToU8a, noop } from '@polkadot/util';
...
export function logger(origin: string): Logger {
  ...
  return {
    debug: isDebug ? (...values: unknown[]) => apply('debug', type, values, maxSize) : noop,
