import { SmProvider } from '@dedot/providers';
...
export const run = async (nodeName: any, networkInfo: any): Promise<any> => {
  ...
  const wsClient = await JsonRpcClient.new(endpoint);
  const spec = await wsClient.rpc.sync_state_genSyncSpec(true);
  await wsClient.disconnect();

  const sm = start();
  const chain = await sm.addChain({ chainSpec: JSON.stringify(spec) });

  const client = await JsonRpcClient.new({ provider: new SmProvider(chain) });
